...
class RodosThread(QThread):
    """
    A thread for reading and parsing RODOS data, then sending it to the PyQt GUI.
    """
    new_data = pyqtSignal(float, float, float)  # Signal to send x, y, z data to the graph
    ...
    def run(self):
        ...
        def topic_handler(data):
            try:
                _, _, x, y, z = struct.unpack("20sIddd", data)
                self.new_data.emit(x, y, z)  # Emit data to the graph
        ...
        gw_uart = rodos.Gateway(luart)
        gw_uart.run()

        rodos2python.addSubscriber(topic_handler)
        gw_uart.forwardTopic(python2rodos)
